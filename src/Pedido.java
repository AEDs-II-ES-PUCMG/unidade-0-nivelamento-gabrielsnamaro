...
public class Pedido {
	...
	public void mesclarPedido(Pedido outro) {
		ItemDePedido[] itensMesclados = new ItemDePedido[MAX_PRODUTOS];
		int novaQuantidade = 0;
		
		for(int i = 0; i < quantProdutos; i++) {
			int indiceOcorrencia = indiceOcorrenciaDoItem(itens[i], itensMesclados, i);
			if(indiceOcorrenciaDoItem(itens[i], itensMesclados, i) == -1) {
				itensMesclados[novaQuantidade] = itens[i];
				novaQuantidade++;
			} else {
				itensMesclados[indiceOcorrencia].mesclarItensDePedido(itens[i]);
			}
		}

		for(int i = 0; i < quantProdutos + outro.quantProdutos; i++) {
			int indiceOcorrencia = indiceOcorrenciaDoItem(outro.itens[i], itensMesclados, i);
			if(indiceOcorrenciaDoItem(outro.itens[i], itensMesclados, i) == -1) {
				itensMesclados[novaQuantidade] = outro.itens[i];
				novaQuantidade++;
			} else {
				itensMesclados[indiceOcorrencia].mesclarItensDePedido(outro.itens[i]);
			}
		}

		if(novaQuantidade > MAX_PRODUTOS)
			throw new UnsupportedOperationException("A quantidade de itens diferentes após a mesclagem é maior que o permitido.");

		itens = itensMesclados;
		dataPedido = dataPedido.isAfter(outro.dataPedido) ?
						dataPedido :
						outro.dataPedido;
		quantProdutos = novaQuantidade;

		outro.itens = new ItemDePedido[MAX_PRODUTOS];
		outro.quantProdutos = 0;
	}

	/**
	 * Retorna -1 se o passado item não está presente no vetor. Se estiver, retorna o índice da ocorrência no vetor.
	 * 
	 * @param item
	 * @param vetor
	 * @param tamanhoMaximo
	 * @return
	 */
	private int indiceOcorrenciaDoItem(ItemDePedido item, ItemDePedido[] vetor, int tamanhoMaximo) {
		for(int i = 0; i < tamanhoMaximo; i++) {
			if(item.equals(vetor[i])) return i;
		}
	...
	public String imprimirRecibo() {
		...
		StringBuilder builder = new StringBuilder("-=-=-=-=- RECIBO -=-=-=-=-\n");

		for(int i = 0; i < quantProdutos; i++) {
			builder.append(itens[i].linhaDeRecibo() + "\n");
		}

		builder.append("\n* TOTAL GERAL: " + moeda.format(valorFinal()));
